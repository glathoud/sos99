...
void main()
{
      ...
      foreach (i; 0..LENGTH-1)
        {
          ...
          // variation: additional early test
          immutable i_early = cast(size_t)( round( 0.66 * (cast(double)( i )) ));
          immutable max_before_early = 0 < i_early  &&  i_early < i  ?  arr[ 0..i_early ].reduce!max  :  double.nan;
          
          immutable selected = (){
            
            auto x2 = arr[ i+1 ];

            // variation: additional early test for top-tier
            // (similar to the standard Secretary Problem Solution)
            if (isFinite( max_before_early ))
              {
                foreach (x; arr[ i_early+1..i ] )
                  {
                    if ((cast(double)( x )) > 0.90 * (cast(double)( max_before_early )))
                      {
                        ++n_early;
                        return x;
                      }
  ...
  writefln( "n_total: %d  n_early: %d (%.2f%%)", n_total, n_early, cast(double)( n_early ) / cast(double)( n_total ) );
  
  const acc_score = score.chunks( N_ITER ).map!((c0) {
      auto c = c0.array.dup.sort.array;
      return c[ cast(size_t)( round( .001 * cast( double )( LENGTH )) )];
    }).array;
  ...
  const acc_score_top_median = score.chunks( N_ITER ).map!((c0) {
      auto c = c0.array.dup.sort.array;
      return c[ cast(size_t)( round( (.001 + 0.999/2.0) * cast( double )( LENGTH )) )];
    }).array;
